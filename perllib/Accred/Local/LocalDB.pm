...
sub new { # Exported
  ...
  #warn "COUCOU:Accred::Local::LocalDB:dbkey = $dbkey, dbhost = $dbhost\n"; printstack ();
  $self->{db} = {
    host => $dbhost,
    name => $dbname,
    user => $dbuser,
...
sub pingdb {
  ...
  my $dbh = $db->{dbh};
  unless ($dbh) {
    warn "Accred::Local::LocalDB::Initializing connection.\n" if $self->{verbose};
...
sub dbconnect {
  ...
  warn "Accred::Local::LocalDB::dbconnect ($db->{host}, $db->{name}, $db->{user}, ".
       "utf8 = $self->{utf8}).\n" if $self->{verbose};
