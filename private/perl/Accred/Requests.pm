...
sub makecsrftoken {
  ...
  error ($self, msg('NoCryptoModule')) unless $aes;

  my  $value = pack ('IC4a*', time, split (/\./, $host), $userid);
  my $padlen = 16 - length ($value) % 16;
  $padlen = 0 if ($padlen == 16);
  $value .= "\0" x $padlen;
  my $token = $aes->encrypt ($value);
...
sub checkcsrftoken {
  ...
  my $userid = $self->{userid};
  my   $host = $self->{remaddr};
  error ($self, msg('InternalError', "Bad call2: /$host/$userid/")) unless ($host && $userid);

  my $csrftoken = $self->{args}->{csrftoken};
  error ($self, msg('NoCSRFToken')) unless $csrftoken;
  ...
  error ($self, msg('BadCSRFToken')) unless $result;
  $result =~ s/\0*$//;

  my ($stamp, $ip1, $ip2, $ip3, $ip4, $keyuser) = unpack ('IC4a*', $result);
  my $keyhost = join ('.', $ip1, $ip2, $ip3, $ip4);
  $keyuser  =~ s/\0*$//;
  error ($self, msg('BadCSRFToken'))     if ($keyuser ne $userid);
  error ($self, msg('BadCSRFToken'))     if ($keyhost ne $host);
  error ($self, msg('ExpiredCSRFToken')) if ($stamp < time - 86400);
  return 1;
}
...
sub initcsrf {
  $needscsrfshield = {
      ...
      # properties
      #
      changeaccredpropertiesform => 1,
          changeaccredproperties => 1,
      #
      # workflows
      #
                     addworkflow => 1,
      ...
      #
                 ...
                 addpropertyform => 1,
                  deleteproperty => 1,
                     modproperty => 1,
                 modpropertyform => 1,
              modunitspolicyform => 1,
                  modunitspolicy => 1,
              modclasspolicyform => 1,
                  modclasspolicy => 1,
                   addunitpolicy => 1,
                   delunitpolicy => 1,
                  addclasspolicy => 1,
